...
@jit(nopython=False)
def apply_kernels(sigmas, r2_maps, n_scales, image, idx, granularity):

    octave = np.zeros(
            (n_scales + 3,) + image.shape
        )
    octave[0] = image
    oi = 0
    ...
    #print(type(self.kernels[idx]))

    #print(len(self.kernels[idx].values()), self.n_scales + 3)
    for kernel in sigmas:
        new_image = np.zeros_like(image)
        #print(new_image.shape)
        bins = np.unique(r2_map)[::granularity]
        if bins[-1] != np.max(r2_map):
            bins = np.append(bins, np.max(r2_map))
        for i in (range(len(bins) - 1)):
            lb = bins[i]
            ub = bins[(i + 1) % len(bins)]
            mask = (lb <= r2_map) & (r2_map <= ub)
            #print(new_image.shape, r2_map.shape)
            new_image += ndi.convolve1d(image, kernel[ub], mode = 'reflect', axis = 1) * mask
        #image = new_image
        ...
        for i in (range(len(bins) - 1)):
            ...
            ub = bins[(i + 1) % len(bins)]
            mask = (lb <= r2_map) & (r2_map <= ub)
            new_image2 += ndi.convolve1d(new_image, kernel[ub], mode = 'reflect', axis = 0) * mask
        octave[oi] = new_image2
        oi += 1
...
class SIFT(FeatureDetector, DescriptorExtractor):
    ...
    def _create_1d_gaussians(self, image_shape, xi, center = None, truncate = 4.0):
        ...
        for kernel in tqdm(self.kernels[idx].values()):
            ...
            for i in range(h):
                for j in range(w):
                    ...
                    convolved = ndi.convolve1d(row, ker, mode = 'reflect')[j] 
            ...
            for k in range(h):
                for m in range(w):
                    ...
                    kslice = ker[rad_ker - left: rad_ker + right]
                    convolved = ndi.convolve1d(row, ker, mode = 'reflect')[k] 
    ...
    def _create_rd_scalespace(self, image):
        ...
        for o in tqdm(range(self.n_octaves)):
    ...
    def _create_scalespace(self, image):
        ...
        for o in range(self.n_octaves):
            ...
            octave[0] = image
            for s in tqdm(range(1, self.n_scales + 3)):
