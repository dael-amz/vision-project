...
def generate_distorted_image(
    ...
    border_mode: int = cv2.BORDER_CONSTANT#cv2.BORDER_REFLECT101 #,
    ...
    if norm_scale is None:
        norm_scale = max(h, w) * 0.5
        #norm_scale = np.sqrt((h/2)**2+(w/2)**2)
...
def undistort_image(
    ...
    # Inverse division model from distorted -> undistorted
    denom = (1.0 + np.sqrt(1 - 4 * xi * rd2)) / 2
...
def water(
    ...
    xu[valid] = (H * xd[valid] + D * A * xd[valid] / denom[valid]) * (1/ (H+D))
    yu[valid] = (H * yd[valid] + D * A * yd[valid] / denom[valid]) * (1/ (H+D))
...
def make_division_distortion_func(
    ...
    center: Optional[Tuple[float, float]] = None,
    norm_scale: Optional[float] = None,
    scale_mode: Literal["none", "srd"] = "none",
):
    """
...
# dist_gray, _, _ = water(gray, H=H, D=D)
# dedist_gray, _, _ = undistort_image(gray, xi)

# undist_gray = gray

# #undist_gray, _, _ = generate_distorted_image(gray, xi)
# #undist_gray = ndi.gaussian_filter(undist_gray, 1.6)
...
# cv2.imshow('SIFT Keypoints', undist_gray )
# cv2.waitKey(0)
# cv2.destroyAllWindows()

# cv2.imshow('SIFT Keypoints', (dist_gray - undist_gray).astype(np.uint8))
# cv2.waitKey(0)
# cv2.destroyAllWindows()
...
# unique = np.unique(rd2)

# results = np.zeros_like(unique)

# for r in (range(len(unique))):
#     mask = rd2 < unique[r]
#     results[r] = np.mean(np.abs(dist_gray[mask] - undist_gray[mask]))
#     print(np.mean(np.abs(dist_gray[mask] - undist_gray[mask])))




# plt.plot(unique, results)
# plt.show()
